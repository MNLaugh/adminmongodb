...
router.get('/api/extmonitoring/:conn', function (req, res, next) {
    if (!req.query || !req.query.apiToken || req.query.apiToken !== req.nconf.app.store.app.apiToken) return res.status(400).json({'msg': req.i18n.__('Token api undefined')});
    next();
}, monit, function (req, res, next){
    if (!req.monitData) res.status(400).json({'msg': req.i18n.__('Could not get server monitoring')});
    else res.status(200).json(req.monitData)
    // https://mongodb.devartsite.com/api/extmonitoring/Main?apiToken=ZFTeoPes7mJY37gYMaoXFX8oy38WUzLN
});
...
router.get('/api/monitoring/:conn', monit, function (req, res, next){
    if (!req.monitData) res.status(400).json({'msg': req.i18n.__('Could not get server monitoring')});
    else res.status(200).json(req.monitData)
});
...
function monit (req, res, next) {
    ...
    req.db.find({connectionName: req.params.conn, eventDate: {$gte: dayBack}}).sort({eventDate: 1}).exec(function (err, serverEvents){
        ...
        if(serverEvents.length > 0){
            ...
            var dataPointsLimit = req.query.limit || 1000;
