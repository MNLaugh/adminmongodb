...
router.get('/app/login', function (req, res, next){
    ...
    req.dbBlacklist.find({ip: ip}, function (err, doc){
        if (err){
            console.error(err)
        ...
        if(doc && doc.length > 0){
            ...
            if(passwordConf && passwordConf.hasOwnProperty('password')){
                if(req.body.inputPassword === passwordConf.password){
                    ...
                    if(req.session.try && req.session.try === 3){
                        req.dbBlacklist.insert({ip: ip}, function (errInsert, newDocs) {
                            if (errInsert){
                              ...
                              return res.render('login', {
                                    message: 'You are now blacklisted!',
                                    helpers: req.handlebars.helpers
                                });
                            }
                        });
                    } else {
                        if (!req.session.try) req.session.try = 1
                        else if (req.session.try < 3) req.session.try++
