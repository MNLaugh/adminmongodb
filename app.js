...
var i18n = new (require('i18n-2'))({
    locales: ['en', 'de', 'es', 'ru', 'zh-cn', 'it', 'fr'],
...
var dbBlacklist = new Datastore({filename: path.join(dir_base, 'data/dbBlacklist.db'), autoload: true});
    ...
    helpers: {
        ...
        app_context: function (){
            if(nconf.stores.app.get('app:context') !== undefined){
                if (nconf.stores.app.get('app:context') !== '') return'/' + nconf.stores.app.get('app:context');
                else return ''
            }return'';
        },
...
var app_context = '';
if(nconf.stores.app.get('app:context') !== undefined){
    if (nconf.stores.app.get('app:context') !== '') app_context = '/' + nconf.stores.app.get('app:context');
    else app_context = ''
}
